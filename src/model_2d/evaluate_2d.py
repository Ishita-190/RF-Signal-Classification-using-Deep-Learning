...
def load_trained_model(device, checkpoint_path=BEST_MODEL_PATH, num_classes=8):
...
def evaluate(
    checkpoint_path=BEST_MODEL_PATH, batch_size=8, hop_length=2048, num_workers=0
):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    X, y = load_dataset()
    _, _, test_loader = create_dataloaders(
        X, y, batch_size=batch_size, num_workers=num_workers, hop_length=hop_length
    )
